   ...
   private void ejecutar(Nodo instrucciones) throws Exception {
        ...
        while (i < instrucciones.hijos.size()) {
            ...
            if (instruccion.valor.equals("asignacion")) {
                ...
                // Ejecutar la asignación inicial
                    ejecutar(instruccion.hijos.get(0));
                 // Evaluar la condición y ejecutar el cuerpo mientras sea verdadera
                 while ((Boolean) evaluarExpresion(instruccion.hijos.get(1))) {
                      ejecutar(instruccion.hijos.get(3)); // Ejecutar el cuerpo
                    ejecutar(instruccion.hijos.get(2)); // Ejecutar el incremento
                 }
            }
                ...
                while ((Boolean) evaluarExpresion(condicion)) {
                    ...
                    Object valor = evaluarExpresion(instruccion.hijos.get(0)); // Evaluar la expresión del switch
                   ...
                   Object valorCaso = evaluarExpresion(caso.hijos.get(0));
                    ...
                    // Ejecutar el caso default si no se ejecutó ningún caso
                if (!ejecutado && instruccion.hijos.get(2).hijos.size() > 0) {
                    ejecutar(instruccion.hijos.get(2).hijos.get(0));
                }
            }
